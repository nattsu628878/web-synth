/**
 * 1次CRローパスフィルタ (one-pole LPF)
 * y[n] = a*x[n] + (1-a)*y[n-1], a = 1 - exp(-2π*fc/fs)
 */

class OnePoleLpfProcessor extends AudioWorkletProcessor {
  ...
  process(inputs, outputs, parameters) {
    ...
    for (let ch = 0; ch < Math.min(input.length, output.length); ch++) {
      ...
      let yPrev = ch === 0 ? this.yPrevL : this.yPrevR;
      ...
      if (ch === 0) this.yPrevL = yPrev;
      else this.yPrevR = yPrev;
    }
...
registerProcessor('one-pole-lpf', OnePoleLpfProcessor);
