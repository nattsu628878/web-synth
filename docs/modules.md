...
各ファイルはモジュールファクトリを export し、`module-registry.js` 経由で `registerModule()` により rack.js に登録する。base.js は契約・型定義のみで登録対象ではない。
...
- **module-registry.js** が各モジュールを `./modules/source/xxx.js` 等で import し、`main.js` の起動時に一括 `registerModule()` する。
