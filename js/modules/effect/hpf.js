/**
 * hpf.js
 * Web Synth - HPF（1次/2次CRハイパスフィルター）
...
function buildHpfDomOnly(title, freqValue, freqLabel) {
...
export const hpfModule = {
  meta: {
    id: 'hpf',
    name: 'HPF',
    kind: 'effect',
    description: '1st/2nd-order RC high-pass filter (Freq, Order)',
    previewDescription: 'Signal: audio in/out.\nHigh-pass; cutoff, order 1/2/4.',
  },

  create(instanceId) {
    const isPreview = String(instanceId).startsWith('preview-');
    if (isPreview) {
      return { element: buildHpfDomOnly('HPF', freqToValue(200), formatFreq(200)) };
    }
    const ctx = ensureAudioContext();
    const inputGain = ctx.createGain();
    inputGain.gain.value = 1;
    const filter1 = new AudioWorkletNode(ctx, 'one-pole-hpf', {
      numberOfInputs: 1,
      numberOfOutputs: 1,
      parameterData: { cutoff: 200 },
    });
    const filter2 = new AudioWorkletNode(ctx, 'two-pole-hpf', {
      numberOfInputs: 1,
      numberOfOutputs: 1,
      parameterData: { cutoff: 200 },
    });
    const filter4 = new AudioWorkletNode(ctx, 'four-pole-hpf', {
      numberOfInputs: 1,
      numberOfOutputs: 1,
      parameterData: { cutoff: 200 },
    });
    ...
    const root = createModuleRoot(instanceId, 'HPF', 'synth-module--effect');
    root.appendChild(createModuleHeader(hpfModule.meta.name));
